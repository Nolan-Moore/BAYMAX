...
def eulerian_magnification(video_filename, image_processing='gaussian', freq_min=0.833, freq_max=1, amplification=50, pyramid_levels=4):
    ...
    file_name = '/home/pi/BAYMAX/Results/resultVideo' #file_name + "_min"+str(freq_min)+"_max"+str(freq_max)+"_amp"+str(amplification)
    combine_pyramid_and_save(vid_data, orig_vid, pyramid_levels, fps, save_filename=file_name + '_magnified.avi')

def get_pulse(video_filename, bounds=None):
    ...
    counted = 0
    for i in averages:
        if i <= numpy.mean(averages) and counted == 0:
            count = count + 1
            counted = 1
        if i >= numpy.mean(averages):
            counted = 0

    target = open('/home/pi/BAYMAX/Results/pulse.txt','w')
    target.truncate()
    pulse = count*6
    target.write("%d bpm" % (pulse))
    target.close()
    print(pulse)
...
def show_frequencies(video_filename, bounds=None):
    ...
    pyplot.plot(frequencies, numpy.real(scipy.fftpack.fft(averages)))
